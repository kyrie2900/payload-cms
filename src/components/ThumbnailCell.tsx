...
const ThumbnailCell: React.FC<ThumbnailCellProps> = ({ cellData }) => {
  if (!cellData) {
    return (
      <div style={{
        width: '60px',
  ...
  // URL转换逻辑 - 与AvatarCell相同
  const convertThumbnailUrl = (originalUrl: string): string => {
  ...
  const imageUrl = convertThumbnailUrl(cellData)
